...
_f.Frm.prototype.save = function (save_action, callback, btn, on_error) {
	...
	var me = this;
	setTimeout(function () {
		me._save(save_action, callback, btn, on_error);
	}, 100);
};
...
_f.Frm.prototype._save = function (save_action, callback, btn, on_error) {
	...
	if (save_action != "Update") {
		...
		$.when(this.script_manager.trigger("validate"), this.script_manager.trigger("before_save")).done(function () {
...
frappe.ui.form.Dashboard = Class.extend({
	init: function init(opts) {
		$.extend(this, opts);
		this.wrapper = $(frappe.render_template('form_dashboard', { frm: this.frm })).prependTo(this.frm.layout.wrapper);
	...
	add_comment: function add_comment(text, permanent) {
		var me = this;
	...
	set_headline_alert: function set_headline_alert(text, alert_class) {
		if (text) {
...
frappe.ui.form.ScriptManager = Class.extend({
	...
	trigger: function trigger(event_name, doctype, name, callback) {
		var me = this;
		...
		return $.when.apply($, $.map(handlers, function (fn) {
			return fn();
		}));
	},
	get_handlers: function get_handlers(event_name, doctype, name, callback) {
		var handlers = [];
		var me = this;
		if (frappe.ui.form.handlers[doctype] && frappe.ui.form.handlers[doctype][event_name]) {
			$.each(frappe.ui.form.handlers[doctype][event_name], function (i, fn) {
				handlers.push(function () {
					return fn(me.frm, doctype, name);
				});
			});
		}
		if (this.frm.cscript[event_name]) {
			handlers.push(function () {
				return me.frm.cscript[event_name](me.frm.doc, doctype, name);
			});
		}
		if (this.frm.cscript["custom_" + event_name]) {
			handlers.push(function () {
				return me.frm.cscript["custom_" + event_name](me.frm.doc, doctype, name);
			});
		}
		return handlers;
	},
...
frappe.ui.form.Grid = Class.extend({
	...
	get_grid_row: function get_grid_row(docname) {
...
frappe.ui.form.Timeline = Class.extend({
	...
	make: function make() {
		...
		this.comment_button = this.wrapper.find(".btn-comment").on("click", function () {
			me.add_comment(this);
		});
	...
	render_timeline_item: function render_timeline_item(c) {
		...
		var $timeline_item = $(frappe.render_template("timeline_item", { data: c, frm: this.frm })).appendTo(me.list).on("click", ".close", function () {
			...
			var update_comment = function update_comment() {
			...
			if (content.hasClass(is_editing)) {
				...
				var $edit_btn = $(this);
				var editing_textarea = me.input.clone().removeClass('comment-input');
	...
	prepare_timeline_item: function prepare_timeline_item(c) {
		...
		if (c.subject && c.communication_type === "Communication" && !frappe._in(this.frm.doc.subject, c.subject) && !frappe._in(this.frm.doc.name, c.subject) && !frappe._in(this.frm.doc[this.frm.meta.title_field || "name"], c.subject)) {
			c.show_subject = true;
		}
	},
	...
	add_comment: function add_comment(btn) {
	...
	insert_comment: function insert_comment(comment_type, comment, btn, input) {
		var me = this;
	...
	setup_mentions: function setup_mentions() {
		this.setup_awesomplete_for_mentions();
	},

	setup_awesomplete_for_mentions: function setup_awesomplete_for_mentions() {
		var me = this;
...
frappe.ui.form.QuickEntryForm = Class.extend({
	init: function init(doctype, success_function) {
		this.doctype = doctype;
	...
	validate_quick_entry: function validate_quick_entry() {
	...
	validate_mandatory_length: function validate_mandatory_length() {
	...
	validate_for_child_table: function validate_for_child_table() {
	...
	insert_document: function insert_document(values) {
		var me = this;
		frappe.call({
