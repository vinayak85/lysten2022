...
frappe.ui.BaseList = Class.extend({
	...
	update_standard_filters: function update_standard_filters(filters) {
		...
		for (var key in this.page.fields_dict) {
			...
			if (value) {
				filters.push([me.doctype, field.df.fieldname, ['Data', 'Text', 'Small Text', 'Text Editor'].includes(field.df.fieldtype) ? 'like' : '=', value]);
			}
		}
	},
...
frappe.ui.FilterList = Class.extend({
	...
	add_filter: function add_filter(doctype, fieldname, condition, value, hidden) {
		if (this.base_list.page.fields_dict[fieldname] && ['=', 'like'].includes(condition)) {
...
frappe.ui.Filter = Class.extend({
	...
	set_field: function set_field(doctype, fieldname, fieldtype, condition) {
		...
		me.field = f;
		if (old_text && me.field.df.fieldtype === cur.fieldtype) me.field.set_input(old_text);
...
frappe.views.Calendar = Class.extend({
	...
	add_filters: function add_filters() {
		var me = this;
		if (this.filters) {
	...
	set_filter: function set_filter(doctype, value) {
		var me = this;
		if (this.filters) {
			$.each(this.filters, function (i, df) {
				if (df.options === value) me.page.fields_dict[df.fieldname].set_input(value);
				return false;
			});
		}
	},
	get_filters: function get_filters() {
		...
		if (this.filters) {
			$.each(this.filters, function (i, df) {
				filter_vals[df.fieldname || df.label] = me.page.fields_dict[df.fieldname || df.label].get_parsed_value();
			});
		}
		return filter_vals;
	},
